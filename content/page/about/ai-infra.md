...
近7年平台研发经验，其中5年+聚焦云原生与AI基础设施平台建设，长期负责资源纳管、任务调度、监控告警、分布式计算与平台工程化落地。在华为数据底座项目中，持续推进异构资源纳管体系建设（历史版本记录覆盖300+集群、500+GPU卡、1000+实例），推动资源利用率提升30%+、成本降低20%+；同时围绕 Ray / KubeRay / RayService / Volcano / Dask 构建 AI 任务调度、离线推理加速、模型发布与可观测能力，并参与 V100 / IB 高性能 Kubernetes 集群纳管、Dask on Ray 与 RDMA 加速实践。

当前求职方向以云原生运维开发与AI Infra平台工程为主，强调“开发驱动运维”的平台化建设能力：通过自动化、标准化和可观测体系，提升平台稳定性、资源效率与交付速度。

## 职业定位

- 目标岗位：AI Infra 工程师、MLOps 平台工程师、云原生运维开发工程师。
- 能力重心：Kubernetes 平台化、GPU/异构资源调度、任务编排、可观测与稳定性治理。
- 交付风格：从需求拆解、方案设计、工程实现到上线运营的全链路闭环。

## 关键成果

- 异构资源治理：历史版本记录覆盖 300+ 集群、500+ GPU 卡、1000+ 数据服务实例的统一纳管与监控。
- 平台稳定性提升：监控采集提升至分钟级，故障发现时效由平均 0.5 天缩短至 5 分钟以内。
- 资源效率优化：历史版本记录推动资源利用率提升 30%+，整体成本降低 20%+。
- AI 任务平台化：围绕 Ray / KubeRay / Volcano 建设任务提交、编排、调度、弹性与生命周期管理能力。
- 推理链路优化：在离线推理和分布式计算方向沉淀可复用实践，支撑 AI 任务高效运行与交付。
...
- **Kubernetes方向**：已通过 CKAD，持续推进 CKA、CKS 体系化学习，强化集群管理、安全加固与云原生安全能力。
- **AI大模型方向**：已完成阿里云 ACP 大模型认证，持续深化 LLM、RAG、Agent、Prompt Engineering 与企业级 AI 平台工程实践。
